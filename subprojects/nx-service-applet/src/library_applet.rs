...
//! that fits its shape (payload storages in, at most one reply out) needs
...
//! 3. Push the common arguments *first*, then the applet-specific payloads.
//!    Every library applet reads its arguments as storage 0, so pushing a
//!    payload first silently misaddresses every field.
