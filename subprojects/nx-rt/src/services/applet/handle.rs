...
// Handle types for LibraryApplet / SystemApplet / OverlayApplet /
// SystemApplication exist as API surface awaiting their FFI shims; only
// ApplicationHandle has a current consumer (`ffi/applet.rs`). Suppress the
// dead-code lint at the module level rather than annotating each typestate
// the macro generates.
#![allow(dead_code)]
