...
/// libnx-faithful runtime handshake.
///
/// # Panics
///
...
// =====================================================================
//
// Each `as_<role>` is consumed by FFI shims in `crate::ffi::applet` (only
// when the `ffi` feature is enabled). Other Meson presets compile
// `service-applet` without `ffi`, momentarily leaving these unused.

/// Returns a typed handle if the applet is initialized as
/// [`AppletType::Application`].
#[cfg_attr(not(feature = "ffi"), allow(dead_code))]
pub(crate) fn as_application() -> Option<ApplicationHandle> {
    let guard = state().read();
    match guard.as_ref() {
...
/// [`AppletType::LibraryApplet`].
#[allow(dead_code)] // API surface; FFI shims for LibraryApplet not yet wired
pub(crate) fn as_library_applet() -> Option<LibraryAppletHandle> {
    let guard = state().read();
    match guard.as_ref() {
...
/// Returns a typed handle if the applet is initialised as
/// [`AppletType::SystemApplet`].
#[allow(dead_code)] // API surface; FFI shims for SystemApplet not yet wired
pub(crate) fn as_system_applet() -> Option<SystemAppletHandle> {
    let guard = state().read();
    match guard.as_ref() {
        // SAFETY: see `as_application`.
...
/// Returns a typed handle if the applet is initialised as
/// [`AppletType::OverlayApplet`].
#[allow(dead_code)] // API surface; FFI shims for OverlayApplet not yet wired
pub(crate) fn as_overlay_applet() -> Option<OverlayAppletHandle> {
    let guard = state().read();
    match guard.as_ref() {
        // SAFETY: see `as_application`.
...
/// Returns a typed handle if the applet is initialised as
/// [`AppletType::SystemApplication`].
#[allow(dead_code)] // API surface; FFI shims for SystemApplication not yet wired
pub(crate) fn as_system_application() -> Option<SystemApplicationHandle> {
    let guard = state().read();
    match guard.as_ref() {
        // SAFETY: see `as_application`.
