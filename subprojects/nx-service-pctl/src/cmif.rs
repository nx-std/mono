...
/// The close obligation is handed on: the factory re-addresses the id per call.
pub(crate) fn create_service_legacy(domain: DomainRef<'_>) -> Result<u32, CreateServiceError> {
...
/// Sends PID. Returns the raw sub-object id; the underlying [`DomainObject`]
/// The close obligation is handed on: the factory re-addresses the id per call.
pub(crate) fn create_service(domain: DomainRef<'_>) -> Result<u32, CreateServiceError> {
...
pub(crate) fn is_restriction_enabled(object: DomainObjectRef<'_>) -> Result<u8, DispatchError> {
    dispatch_out(object, proto::IS_RESTRICTION_ENABLED)
}

/// Gets the current safety level.
pub(crate) fn get_safety_level(object: DomainObjectRef<'_>) -> Result<u32, DispatchError> {
...
pub(crate) fn is_stereo_vision_permitted(object: DomainObjectRef<'_>) -> Result<u8, DispatchError> {
    dispatch_out(object, proto::IS_STEREO_VISION_PERMITTED)
}

/// Checks whether pairing is active.
pub(crate) fn is_pairing_active(object: DomainObjectRef<'_>) -> Result<u8, DispatchError> {
...
pub(crate) fn is_play_timer_alarm_disabled(
    object: DomainObjectRef<'_>,
) -> Result<u8, DispatchError> {
