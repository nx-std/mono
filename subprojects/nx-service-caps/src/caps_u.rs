...
//! current application.
//!
//! The service is non-domain: [`connect_capsu_cmif`] obtains the root session,
...
//! [`CapsuService::open_accessor_session`], which returns a [`CapsuAccessor`]
//! owning its own independent session handle.
//!
//! Listing album files is spread over several commands, each exposed directly
//! so a caller picks the one its target firmware has:
//!
...
impl CapsuService {
    ...
    pub fn open_accessor_session(
        ...
        entry: &ApplicationAlbumFileEntry,
        applet_resource_user_id: u64,
    ) -> Result<CapsuAccessor, CapsuOpenAccessorSessionError> {
...
pub fn connect_capsu_cmif(sm: &SmService) -> Result<CapsuService, ConnectCapsuCmifError> {
    let handle = sm
        .get_service_handle_cmif(CAPSU_SERVICE_NAME)
        .map_err(ConnectCapsuCmifError)?;
...
/// Error returned by [`connect_capsu_cmif`].
#[derive(Debug, thiserror::Error)]
#[error("failed to get caps:u service")]
pub struct ConnectCapsuCmifError(#[source] pub nx_service_sm::GetServiceCmifError);
