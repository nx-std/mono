...
pub(crate) fn get_album_file_list(
    ...
    storage: u8,
    entries: &mut [u8],
) -> Result<u64, CapsaGetAlbumFileListError> {
    ...
    let result = service
        ...
        .send(&mut ipc_buf)
        .map_err(CapsaGetAlbumFileListError)?;
...
pub struct CapsaGetAlbumFileListError(#[source] pub DispatchError);
...
/// Wire-layout input for [`storage_copy_album_file`] (cmd 4).
#[derive(Clone, Copy, zerocopy::IntoBytes, zerocopy::Immutable)]
#[repr(C)]
struct StorageCopyAlbumFileIn {
    /// Storage to copy the file to.
    storage: u8,
    /// Padding the wire form carries after the storage.
    _pad: [u8; 7],
    /// File to copy.
    file_id: AlbumFileId,
}
...
/// Wire-layout output for screenshot load commands returning width/height (cmds 9, 10, 12, 13).
...
/// Wire-layout input for [`get_album_entry_from_app_album_entry`] (cmd 11).
#[derive(Clone, Copy, zerocopy::IntoBytes, zerocopy::Immutable)]
#[repr(C)]
struct GetAlbumEntryFromAppEntryIn {
    /// Opaque application entry to resolve.
...
/// Wire-layout input for the screenshot commands that carry decode options
/// (cmds 12, 13, 14, 1001, 1002, 1003).
#[derive(Clone, Copy, zerocopy::IntoBytes, zerocopy::Immutable)]
#[repr(C)]
struct LoadScreenShotExIn {
    /// File holding the image to decode.
    file_id: AlbumFileId,
...
#[derive(Clone, Copy, zerocopy::IntoBytes, zerocopy::Immutable)]
#[repr(C)]
struct StorageFlagsIn {
    /// Storage the command applies to.
    storage: u8,
    /// Padding the wire form carries after the storage.
    _pad1: [u8; 7],
    /// [`AlbumFileContentsFlag`](crate::AlbumFileContentsFlag) bits selecting
    /// which file types the command covers.
    flags: u8,
    /// Padding the wire form carries after the flags.
    _pad2: [u8; 7],
}
...
/// Wire-layout output for [`get_min_max_applet_id`] (cmd 18).
#[derive(Clone, Copy, zerocopy::FromBytes, zerocopy::Immutable, zerocopy::KnownLayout)]
#[repr(C)]
pub(crate) struct GetMinMaxAppletIdOut {
    /// Non-zero when the service found a range to report.
    pub success: u8,
    /// Padding the wire form carries after the flag.
    pub _pad: [u8; 3],
}
...
pub(crate) struct GetLastOverlayThumbnailOut {
    /// File the thumbnail was taken from.
...
/// Wire-layout input for [`get_required_storage_space_size_to_copy_all`] (cmd 501).
#[derive(Clone, Copy, zerocopy::IntoBytes, zerocopy::Immutable)]
#[repr(C)]
struct GetRequiredStorageSizeIn {
    /// Storage the files would be copied to.
    dst_storage: u8,
    /// Storage the files would be copied from.
    src_storage: u8,
}
...
/// Wire-layout input for [`get_album_cache_ex`] (cmd 8013).
#[derive(Clone, Copy, zerocopy::IntoBytes, zerocopy::Immutable)]
#[repr(C)]
struct GetAlbumCacheExIn {
    /// Storage the cache is queried for.
    storage: u8,
    /// Content type the cache is queried for.
    contents: u8,
}
...
/// Wire-layout input for [`get_album_entry_from_app_album_entry_aruid`] (cmd 8021).
#[derive(Clone, Copy, zerocopy::IntoBytes, zerocopy::Immutable)]
#[repr(C)]
struct GetAlbumEntryFromAppEntryAruidIn {
    /// Opaque application entry to resolve.
    application_entry: ApplicationAlbumEntry,
...
pub(crate) fn open_accessor_session(
    service: &Session,
    aruid: u64,
) -> Result<u32, CapsaOpenAccessorSessionError> {
    let mut ipc_buf = nx_sys_thread_tls::ipc_buffer();
    ...
    let result = service
        ...
        .send_pid()
        .send(&mut ipc_buf)
        .map_err(CapsaOpenAccessorSessionError::Dispatch)?;

    if result.move_handles.is_empty() {
        return Err(CapsaOpenAccessorSessionError::MissingHandle);
    }
...
#[derive(Debug, thiserror::Error)]
pub enum CapsaOpenAccessorSessionError {
    /// IPC dispatch failed.
...
pub(crate) fn read_movie_data_from_stream(
    ...
    offset: i64,
    buffer: &mut [u8],
) -> Result<u64, CapsaReadStreamDataError> {
    ...
    let result = service
        ...
        .out_size(size_of::<u64>())
        .send(&mut ipc_buf)
        .map_err(CapsaReadStreamDataError)?;
...
pub struct CapsaReadStreamDataError(#[source] pub DispatchError);
