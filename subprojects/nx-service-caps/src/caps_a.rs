...
//!
//! The service is non-domain: [`connect_capsa_cmif`] obtains the root session,
//! its methods are then called directly, and the session is closed on drop.
//! Movie streaming needs an accessor sub-object, obtained from
//! [`CapsaService::open_accessor_session`], which returns a [`CapsaAccessor`]
//! owning its own independent session handle.
...
/// Dimensions of a screenshot decoded by a load-screenshot command.
...
/// Dimensions and attributes of a screenshot decoded by an `Ex0` command.
#[derive(Debug, Clone, Copy)]
...
/// File identifier and size of an overlay thumbnail.
#[derive(Debug, Clone, Copy)]
pub struct OverlayThumbnailResult {
    /// File the thumbnail was taken from.
    pub file_id: AlbumFileId,
    /// Size of the thumbnail, in bytes (always 0x5100).
    pub size: u64,
}

/// Applet ID range reported by [`CapsaService::get_min_max_applet_id`].
#[derive(Debug, Clone, Copy)]
pub struct MinMaxAppletIdResult {
    /// Whether the service found a range to report.
    pub success: bool,
    /// Lowest applet ID in the range.
    pub min: u64,
    /// Highest applet ID in the range.
    pub max: u64,
}
...
impl CapsaService {
    ...
    pub fn open_accessor_session(
        &self,
        aruid: u64,
    ) -> Result<CapsaAccessor, CapsaOpenAccessorSessionError> {
        ...
        Ok(CapsaAccessor(Session::new(handle, 0)))
    }
}
...
pub fn connect_capsa_cmif(sm: &SmService) -> Result<CapsaService, ConnectCapsaCmifError> {
    let handle = sm
        .get_service_handle_cmif(CAPSA_SERVICE_NAME)
        .map_err(ConnectCapsaCmifError)?;
...
/// Error returned by [`connect_capsa_cmif`].
#[derive(Debug, thiserror::Error)]
#[error("failed to get caps:a service")]
pub struct ConnectCapsaCmifError(#[source] pub nx_service_sm::GetServiceCmifError);
