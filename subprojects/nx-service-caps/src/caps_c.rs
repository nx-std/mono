...
//!
//! The service is non-domain: [`connect_capsc_cmif`] obtains the root session,
//! its methods are then called directly, and the session is closed on drop.
//! Movie streaming, read and write alike, needs a control session sub-object,
//! obtained from [`CapscService::open_control_session`], which returns a
//! [`CapscControlSession`] owning its own independent session handle.
//!
//! Commands whose wire format differs across firmware versions are exposed as
//! paired methods, such as
//! [`CapscService::register_applet_resource_user_id_legacy`] and
//! [`CapscService::register_applet_resource_user_id`].
...
pub fn connect_capsc_cmif(sm: &SmService) -> Result<CapscService, ConnectCapscCmifError> {
    let handle = sm
        .get_service_handle_cmif(CAPSC_SERVICE_NAME)
        .map_err(ConnectCapscCmifError)?;
...
/// Error returned by [`connect_capsc_cmif`].
#[derive(Debug, thiserror::Error)]
#[error("failed to get caps:c service")]
pub struct ConnectCapscCmifError(#[source] pub nx_service_sm::GetServiceCmifError);
