/* nx-rt-hbapp — homebrew-loader `.crt0` startup section.
 *
 * This is the homebrew-NRO `_start` for the opt-in `rustc`-driven link
 ...
 *
 * Startup ABI — homebrew loader (hbloader) handoff:
 *   x0 = pointer to the homebrew environment context
 *   x1 = UINT64_MAX (-1) sentinel
...
.Lcrt0_main_entry:
    ...
    mov x25, x0  // hbloader environment context pointer
    mov x26, x1  // UINT64_MAX (-1) sentinel
