//! `libnx` symbol-override FFI for `nx-rt-hbapp`.
//!
//! Holds the `__nx_rt_hbapp__libnx_*` symbols that redirect the loader-handoff
...
//! `overrides/rt_hbapp_libnx_core.ld` and the per-service `overrides/rt_hbapp_libnx_service_*.ld`
//! fragments.
