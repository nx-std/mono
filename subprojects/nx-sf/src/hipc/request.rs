...
//! The payload owns the bytes that go into the data-words region. The trait and
//! its built-in impls live in the `wire` module, because a reply is parametric
//! over the same thing.
//!
...
///
/// Unlike [`HIPC_MAX_DESCRIPTORS`], this one stays here: a receive list is the
/// client's pre-allocation for pointer data, so only the request side has one.
...
// Ungated, unlike the mappings on the protocol layers above: a server answers a
// message it could not even frame by putting this code in the reply, so the
// `server` feature reaches it with `ffi` off.
