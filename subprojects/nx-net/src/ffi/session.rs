...
//! service explicitly, so the FFI owns the resolver session itself. The
//! service-manager session it is acquired over belongs to the runtime, and is
//! borrowed for the length of the handshake rather than opened here. It is
...
pub fn with_resolver<T>(
    sm: &SmService,
    op: impl FnOnce(&SfdnsresService) -> T,
    ...
    let svc: &SfdnsresService = match guard.as_mut() {
        Some(svc) => svc,
        None => guard.insert(resolver::connect(sm)?),
    };
    Ok(op(svc))
}
