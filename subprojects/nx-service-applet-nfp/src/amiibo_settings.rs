...
impl AmiiboSettings<'_> {
    ...
    pub fn start(
        ...
        let mut reply = [0u8; size_of::<ReturnValueForAmiiboSettings>()];
        ...
        // SAFETY-adjacent: the reply buffer is exactly the struct's size and the
        // struct is all-bytes-valid, so this cannot fail.
        let reply = ReturnValueForAmiiboSettings::read_from_bytes(&reply)
            .map_err(|_| StartError::MalformedReply)?;
...
pub enum StartError {
    ...
    /// The reply storage did not hold a whole reply struct.
    #[error("the reply storage was malformed")]
    MalformedReply,
...
impl nx_sf::error::ToResultCode for StartError {
    fn to_rc(self) -> nx_sf::error::ResultCode {
        match self {
            ...
            Self::AbnormalExit(_) | Self::MalformedReply | Self::AppletFailed => {
