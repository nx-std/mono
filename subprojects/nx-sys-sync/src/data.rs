...
//!
//! A generic [`Mutex<T>`] that bundles the allocation-free [`crate::Mutex`]
//! lock with the data it guards, handing out scoped access through an RAII
//! [`MutexGuard`].
//!
//! This is the single authoritative home for the data-wrapper knowledge
//! previously cloned by `nx-alloc` and `nx-sys-virtmem`. It is `core`-only — it
