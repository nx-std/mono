...
use nx_sf::service::{Domain, DomainObjectRef, DomainRef};
...
impl<'a> SessionGuard<'a> {
    ...
    /// Addresses `raw_object_id` inside this pool slot's domain. The object id
    /// is one stashed long-term by the service wrapper (e.g. the
    /// `IFriendService` sub-object), so the view closes nothing: the object
