...
pub fn register_process(
    ...
    let fmt = cmif::RequestFormatBuilder::new(proto::REGISTER_PROCESS)
    ...
    let _resp = unsafe { cmif::parse_response(ipc_buf, false, 0) }
        .map_err(RegisterProcessError::ParseResponse)?;
...
pub fn unregister_process(session: SessionHandle, pid: u64) -> Result<(), UnregisterProcessError> {
    ...
    let fmt = cmif::RequestFormatBuilder::new(proto::UNREGISTER_PROCESS)
        .data_size(size_of::<u64>())
        .build();
