//! Thread creation implementation using Arc+Pin pattern
//!
//! This module provides the core thread creation functionality, porting the
//! `threadCreate` function from libnx's `thread.c` to safe Rust using the
//! Arc+Pin pattern for thread handle management.
...
use crate::{
    thread_exit, thread_handle::Thread, thread_stackmem::ThreadStackMem, tls_block, tls_region,
};
...
/// Creates a new thread using the Arc+Pin pattern
///
/// This is the Rust port of libnx's `threadCreate` function, updated to use
/// the Arc<Pin<ThreadInner>> pattern for safe concurrent thread handle management.
///
/// # Arguments
/// * `entrypoint` - Thread entry point function
...
///
/// Returns `Ok(Thread)` on success, `Err(ThreadCreateError)` on failure.
pub fn create(
    ...
    let stack_top = unsafe {
        (stack_mirror.as_ptr() as *mut u8)
            .add(effective_stack_sz)
            .sub(size_of::<ThreadEntryArgs>())
    };
    let tls_ptr = unsafe { (stack_mirror.as_ptr() as *mut u8).add(effective_stack_sz) };
    let reent_ptr = unsafe { tls_ptr.add(tls_sz) };

    // Create the ThreadStackMem based on ownership
    let thread_stack_mem = if mapped_stack.is_owned() {
    ...
    // Set up thread entry arguments first
    let args_ptr = stack_top as *mut ThreadEntryArgs;
    unsafe {
        args_ptr.write(ThreadEntryArgs {
            thread: ptr::null(), // Will be filled after Thread creation
            entrypoint,
            arg,
    ...
    let handle = svc::create(
        thread_entrypoint_wrapper as *mut c_void,
    ...
    // Create the new Thread using Arc+Pin pattern
    let thread = Thread::new(handle, thread_stack_mem);

    // Update ThreadEntryArgs with the actual thread pointer
    let thread_raw = thread.clone().into_raw();
    unsafe {
        (*args_ptr).thread = thread_raw;
    }

    // Set up child thread's reent struct
    unsafe {
...
/// - Reconstructs the Thread handle from the raw pointer
...
fn thread_entrypoint_wrapper(args: *mut ThreadEntryArgs) {
    ...
    // Reconstruct the Thread from the raw pointer
    let thread = unsafe { Thread::from_raw(args.thread) };
    ...
    // TODO: Register thread with global registry when registry is updated for Arc+Pin

    // Launch thread entrypoint
    unsafe { (args.entrypoint)(args.arg) };

    // Exit thread
    unsafe { thread_exit::exit(thread) };
}

/// Thread creation arguments structure
///
/// Keep this struct's size 16-byte aligned, matching the C version.
/// Updated for Arc+Pin pattern to use raw pointers for FFI compatibility.
#[repr(C, align(16))]
struct ThreadEntryArgs {
    thread: *const (), // Raw pointer from Thread::into_raw()
    entrypoint: ThreadFunc,
    arg: *mut c_void,
    reent: *mut c_void,
    tls: *mut c_void,
