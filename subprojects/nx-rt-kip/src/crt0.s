/* nx-rt-kip — kernel-launch `.crt0` startup section.
 *
 * This is the boot-time-KIP `_start` for the opt-in `rustc`-driven link
 ...
 *
 * Startup ABI — kernel-launch handoff:
 *   x0 = 0
 ...
 *
 * A KIP is launched directly by the kernel: it maps the `KIP1` image segments
 * and jumps here. There is no process-manager (`pm`) handoff and no homebrew-
 * loader environment block — the KIP brings its environment up from nothing.
 * The kernel's main-thread register convention is the same one a `pm`-launched
 * NSO sees (`x0 = 0`, `x1 = main-thread handle`); what differs is that no `pm`
 * mediates the launch and there is no loader to return to — a kernel-launched
 * process exits through the process-exit syscall.
 *
...
.Lcrt0_main_entry:
    // Preserve the kernel-supplied state across the init calls.
    mov x25, x0  // entry-point argument 0 (0 on kernel launch)
    mov x26, x1  // entry-point argument 1 (main-thread handle on kernel launch)
    mov x27, x30 // loader return address (inert — a KIP has no loader)
