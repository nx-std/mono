...
//
// These cases read the command line the program was started with, so what they
// see depends on who started it: the homebrew menu passes the path it launched,
// and so does the runner. The same binary links either way: with `use_nx_rt`
// off the runtime is libnx's own, which is the baseline to compare ours
// against.
...
int main()
{
    ...
    tap_begin("rt", VERSION, RIG_DIR, unattended);
