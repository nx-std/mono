 ...
 *
 * Thread A (highest priority 0x20) acquires lock first.
 * Thread B (lowest priority 0x2C) tries to acquire, blocks.
 * Thread C (medium priority 0x2B) tries to acquire, blocks.
 * When A releases, C (higher priority than B) should acquire next.
 */
test_rc_t test_0005_remutex_multiple_threads_different_priority(void) {
    ...
    // T4: Wait for Thread A to unlock, Thread C (higher priority) acquires
    ...
    // T5: Wait for Thread C to unlock, Thread B acquires
    ...
    // T6: Wait for Thread B to unlock
    ...
    // - T4: Thread A unlocked, Thread C (higher priority) acquired
    if (shared_tag_t4 != THREAD_C_TAG) {
        rc = TEST_ASSERTION_FAILED;
        goto test_cleanup;
    }

    // - T5: Thread C unlocked, Thread B acquired
    if (shared_tag_t5 != THREAD_B_TAG) {
        rc = TEST_ASSERTION_FAILED;
        goto test_cleanup;
    }

    // - T6: Thread B unlocked
