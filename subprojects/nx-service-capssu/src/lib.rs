...
//!
//! libnx's `capssu.c` keeps a guarded global singleton (`g_capssuSrv`) managed
//! by `NX_GENERATE_SERVICE_GUARD`, enforces hosversion checks at each call site,
//! and automatically calls `SetShimLibraryVersion` during initialization on
//! 7.0.0+. This crate follows the convention of the other `nx-service-*` crates:
...
//! 7.0.0+ for [`CapssuService::set_shim_library_version`] and
//! [`CapssuService::save_screen_shot_ex1`], 6.0.0+ for
//! [`CapssuService::save_screen_shot_ex2`]).
//!
//! The convenience wrappers from libnx (`capssuSaveScreenShot`,
//! `capssuSaveScreenShotWithUserData`, `capssuSaveScreenShotWithUserIds`) are
//! not replicated — callers construct [`ScreenShotAttribute`],
//! [`ApplicationData`], or [`UserIdList`] directly and call the `Ex` methods.
...
pub fn connect_cmif(sm: &SmService) -> Result<CapssuService, ConnectCmifError> {
    let handle = sm
