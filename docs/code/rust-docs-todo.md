...
// stranger can act on.
// TODO(dana): split this module into `cmif` and `tipc` submodules — the two
//  request builders below share no items, and a service that speaks one protocol
//  compiles both.
```
...
// TODO(nx-sf-tipc): commands declared as TIPC-only are unsupported.
...
// TODO(nx-std/sys): commands declared as TIPC-only are unsupported.
...
// TODO(dana): commands declared as TIPC-only are unsupported — one is dispatched
//  over CMIF rather than the TIPC path by the request builder.

// ✅ Good — unowned, because the work is small, local, and obvious in place.
// TODO: drop this alias once the last caller moves to `send_request`.
```
...
// ("dispatched over CMIF…") is a fragment that means nothing on its own.
// TODO(dana): commands declared as TIPC-only are unsupported —
// one is dispatched over CMIF rather than the TIPC path.
...
// TODO(dana): commands declared as TIPC-only are unsupported —
//  one is dispatched over CMIF rather than the TIPC path, because the request
//  builder emits CMIF headers only.
```
...
// FIXME: tipc commands go out as cmif

// ✅ Good — one keyword, and the severity lives in the sentence, which says what a
// caller actually loses.
// TODO(dana): commands declared as TIPC-only are unsupported —
//  one is dispatched over CMIF rather than the TIPC path, so a session opened on
//  firmware 12.0.0 or later gets `0x1A80A` back instead of a reply.
```
...
/// Encode a service command into an outgoing session request.
///
/// TODO(dana): commands declared as TIPC-only are encoded as CMIF.
pub fn encode(cmd: Command, buf: &mut [u8]) -> Result<usize, EncodeError> {}
...
/// Encode a service command into an outgoing session request.
// TODO(dana): commands declared as TIPC-only are unsupported — one is encoded
//  with a CMIF header rather than a TIPC one, so a caller that issues it against
//  a TIPC-only session gets a protocol error instead of a reply.
pub fn encode(cmd: Command, buf: &mut [u8]) -> Result<usize, EncodeError> {}
```
...
Ok(unsafe { SessionHandle::from_raw_unchecked(raw) })

// ✅ Good — the justification says why this is correct now. A TODO may sit
// alongside it, but it does not replace it.
// SAFETY: `raw` was returned by a successful `svcConnectToNamedPort`, so it names
// a live session this thread owns and closes exactly once.
Ok(unsafe { SessionHandle::from_raw_unchecked(raw) })
```
