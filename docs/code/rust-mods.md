...
`nx_sf::cmif::request` is `nx-sf/src/cmif/request.rs`, and `nx_svc::mem::page` is `nx-svc/src/mem/page.rs`;
neither path is something a reader has to be told.
...
learning that it did. `cmif` declares `request`, `response`, and `wire` and re-exports `Request`,
`Response`, and `DispatchError` from them; splitting `wire` into two files later is a `cmif` edit and
nothing else.
...
returning to where it started. `mem::page` may reach `mem::permission` beside it, but never back up into
`mem` for the `PageSize` constant it needs.
...
its parent holds — `PageSize` living in `mem.rs` while only `mem::page` uses it — the item is in the wrong
file, and moving it is the fix.
