...
```
// ❌ Bad — every module is a file called mod.rs, so an editor's tab bar and a panic
// location both say "mod.rs" and neither says whether it is hipc, cmif or tipc.
src/
  hipc/
    mod.rs         // HIPC module code and sub-module declarations
    buffer.rs
    wire.rs
```
...
// in a panic location.
src/
  hipc.rs          // HIPC module code and sub-module declarations
  hipc/
    buffer.rs
    wire.rs
```
...
```rust
// ✅ Good — callers depend on `mem::PageAddr`, so moving `PageAddr` between sub-modules
// stays an edit to one re-export line rather than a change at every call site.
...
// In src/mem/page.rs
/// Page-aligned address in the process address space.
...
type only the sub-modules use is not. A sub-module that is itself the surface — an `ffi` module behind its
Cargo feature, whose `__nx_*` functions are named as `mem::ffi::*` — is declared `pub mod` instead.
...
- [ ] Named module files (e.g. `hipc.rs`, `mem.rs`) sit next to directories of the same name
