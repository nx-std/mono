...
Rule documents describe conventions that **exist in `subprojects/`**, not conventions imported from other ecosystems or aspirational ones.
...
| The build or manifest changes   | `rust-crates` / `meson-subproject` |
...
A group's parent doc (`rust-mods`, `rust-docs`) is itself a **rule document with content**, not an index. Do not add a doc whose only job is to route to its siblings: frontmatter discovery already does that, and a hand-maintained routing table is a second source of truth that goes stale silently.

### A Group's Prefix Names Its Subject

A rule filed under a prefix must be **about that subject**. `rust-errors-*` is about how this workspace declares and surfaces errors; a rule that merely _returns_ a `Result` does not belong there. Ask what the rule is about, not what it touches — a doc about a crate's FFI surface is about that FFI surface, however much `thiserror` appears in it.
...
The test is where a reader needs it. Someone editing an SVC wrapper is looking at that wrapper's module, not searching `docs/code/` — so the invariant the kernel ABI forces on it, and the comment that must accompany it, are documented at the declaration. A rule that restates a single module's contract is a copy that drifts from the code it describes.
...
modules and imports are laid out, how code is documented, how tests are organized.
Most rules are `core`.
...
Rules scoped to individual crates, using the `crate-` prefix followed by the crate's directory name under
`subprojects/`, which is also its package name: the doc governing `subprojects/nx-svc` is `crate-nx-svc`. A
security companion takes the same name plus `-security`, and the `scope` field uses the same directory name
(`crate:nx-svc`).

A document governing a family of sibling crates names the family, not one member: `crate-nx-service` covers
every `subprojects/nx-service-*`.
...
- ✅ `"SVC wrapper patterns for the raw supervisor-call surface. Load when working on the nx-svc crate"`
...
(language conventions), `rust-errors-*`, `rust-docs-*`, `test-*`, `meson-*` (this workspace's build layout), and
`crate-*` (crate-scoped). A rule document that fits none of them is standalone, and a new group is created by
writing its first member.

Three of those prefixes read alike and are answered by asking **how many crates the rule is about**:
...
| `meson-*`    | This workspace's build layout as a set                         | Where a subproject lives and how it is wired |
| `crate-*`    | One crate, or one family of sibling crates                     | How that crate's FFI surface is written    |

So a rule that holds for a crate in any Rust project is `rust-*`; a rule that holds only because *this*
workspace is laid out the way it is, is `meson-*`; and a rule that stops being true outside one crate is
`crate-*`.
...
6. **Crate patterns** - Use `crate-` prefix followed by the directory name under `subprojects/`
...
| `Foundation` | Core rule a crate/arch rule builds on | crate-nx-svc -> rust-errors-reporting |
| `Companion` | Paired doc for same crate | crate-nx-svc <-> crate-nx-svc-security |
...
- [crate-nx-svc-security](crate-nx-svc-security.md) - Companion: Security checklist
```

### Examples

- ✅ `rust-mods-members` -> `rust-mods` (Extends: core to core)
- ✅ `crate-nx-svc` -> `rust-errors-reporting` (Foundation: crate to core)
- ✅ `crate-nx-svc-ffi` -> `crate-nx-svc` (Extends: crate to crate)
- ✅ `crate-nx-svc` <-> `crate-nx-svc-security` (Companion: bidirectional)
- ✅ `meson-subproject-crate` -> `rust-mods` (Foundation: arch to core)
- ✅ `test-files` <-> `test-functions` (Related: core siblings)
- ❌ `code` -> `rust-mods` (meta rules only reference other meta rules)
- ❌ `rust-mods` -> `crate-nx-svc` (core cannot reference crate rules)
...
**MANDATORY for ALL `__nx_*` FFI symbol definitions in the workspace**
**MANDATORY for ALL `Cargo.toml` files in the workspace**
**MANDATORY for ALL subproject crates (`subprojects/<name>/`)**
...
This is deliberate, and it is the opposite of what a citation buys. A `// ✅ Good — subprojects/x/src/y.rs`
...
- **Never assert, in prose, that a named module does the thing.** "`subprojects/nx-svc/src/mem.rs`
  states X" is a citation wearing a sentence, and it rots on the next rename. State the rule.
- **Invent the names.** Illustrative subjects (`fetch_user`, `ConnectOptions`, `parse_header`) are
...
- **Third-party and core APIs**: `thiserror`, `zerocopy`, `core::sync::atomic::AtomicU32`. A doc that gets
  these wrong teaches the wrong thing.
- **The names of crates and subprojects.** `nx-svc`, `nx-alloc`, `nx-sys-sync`, `nx-std` — written as they
  really are, never disguised. These are the workspace's
...
- **A workspace crate that a rule names as its subject.** A rule that says "every FFI entry point
  returns a `u32` result code" is stating the convention, not citing a module. The test is whether the
...
`src/lib.rs`, `subprojects/<crate>/src/`, `tests/*.rs`. What is banned is pointing at one real module as
evidence.
...
**A convention must still exist in `subprojects/` to be documented** ([§1](#1-core-principles)) — that
...
- Document a convention no crate in `subprojects/` demonstrates (a convention nothing demonstrates is not one)
...
- [ ] Crate-specific documents follow `crate-<crate-name>.md` format
...
- [ ] Every convention documented is demonstrated by code in `subprojects/` (the author checked; the doc does not cite it)
...
- [ ] No example is a transcription of real code, and a rename anywhere in `subprojects/` could not falsify the doc
