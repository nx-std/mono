...
A `rust-*` document is `"core"`. A rule about manifests, workspace shape, or build configuration governs where
code lives rather than how it is written, which is a different subject and belongs under a prefix that names
it ([code.md §1](code.md#1-core-principles)).
...
- **Invented subjects, real crate names.** Types, functions, fields, and the failure stories in the marker
  comments are fabricated. Crates, services, and binaries keep the names they actually have, in this workspace
...
**This template does not pin their relative order.** [code.md §5](code.md#5-document-structure) lists
Checklist before References, and [code.md §7](code.md#7-template) emits References before Checklist. Until the
base specification settles that, follow whatever `code.md` resolves to and keep the whole group on one order.
This is the only structural question this template leaves open.
...
{{`## References` and `## Checklist` follow here. This template does not pin their relative
order — see Structure > Order of Checklist and References.}}
