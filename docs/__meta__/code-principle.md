...
Examples must be idiomatic for this workspace: edition-current `no_std` Rust, `thiserror` error enums, and `core`-only APIs.
...
Examples show the least invented code that carries the principle, cite no module, and never transcribe real code — no rename anywhere in `subprojects/` may be able to falsify the doc ([code](code.md) §6).
