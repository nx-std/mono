...
Patterns documented here MUST already be used in `subprojects/` — verify that before writing, then write the example from scratch.
...
Examples are fabricated: show the least invented code that carries the pattern, cite no module, and never transcribe real code — no rename anywhere in `subprojects/` may be able to falsify the doc ([code](code.md) §6).
