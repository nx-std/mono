...
| an error type, a `Result`, a discarded error, a retry | `rust-errors-*` |
...
| a `meson.build`, `meson.options`, or `*_override.ld` | `meson-*` |
| a `#![no_std]` crate preamble, or an `ffi` module or `__nx_*` symbol | `rust-no-std`, `rust-ffi` |
| files under a crate that has a `crate-<name>` doc | that doc **and** its `-security` companion |
...
> `subprojects/nx-svc/src/mem.rs:118` — **rust-errors-reporting**: the error enum is declared before the
> function that returns it. Move `MapError` after `map_region`.
