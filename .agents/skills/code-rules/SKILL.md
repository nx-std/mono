...
`docs/code/` holds ~40 rule documents, ~100k tokens in total. This skill loads only their frontmatter, and you
...
- **Take the most specific match.** A prefix is a group (`rust-docs-*`, `rust-fn-*`, `rust-errors-*`,
  `rust-mods-*`, `meson-*`, `test-*`). Read the member whose trigger fits — `rust-docs-rustdoc` for a `///`
  block, not the group. Add the parent only when the task turns on what the parent owns: `rust-docs` decides
  which channel a fact belongs in, `rust-mods` decides which file an item belongs in.
- **Expect two to four documents.** One is common. More than four means the task is unscoped, or you are
  matching topics instead of triggers. Crate work is the exception: a `crate-*` doc, its `-security`
  companion, and a core doc or two.
- **Break ties by specificity:** `scope: crate:<name>`, then `type: core`, then `arch`, then `principle`.
  Read `type: meta` only when writing a rule document itself.
...
| `/code-format`, `/code-check`, `/code-build`, `/code-deploy`, `/code-test` | running formatters, lints, builds, tests |
