---
name: commit
description: Generate and validate conventional commit messages with Rust workspace scope rules. Use when user says "commit", "git commit", "/commit", asks for help with commit messages, or requests to amend a commit. Enforces crate-based scoping, validates message accuracy against changesets, and forbids AI attribution.
allowed-tools: Bash(git * status *), Bash(git * diff *), Bash(git * show *), Bash(git * log *), Bash(git * shortlog *), Bash(git * ls-files *), Bash(git * ls-tree *), Bash(git * diff-tree *), Bash(git * rev-parse *), Bash(git * describe *), Bash(git * blame *), Bash(git * cat-file *)
---
...
**NEVER add ANY AI attribution in commit messages. This is MANDATORY.**
...
- ❌ "AI-assisted"
- ❌ Any mention of Claude, AI tools, or automated generation
- ❌ Any links to Claude Code or Anthropic

**Commits MUST be completely clean of ALL AI attribution.**

The user must feel complete ownership. NEVER violate this rule.
